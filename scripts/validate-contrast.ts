  ...
  for (const demographicId of demographics) {
    ...
    try {
      ...
      if (demographicId === 'education') {
        ...
        console.log('DEBUG: Education brand palette:', JSON.stringify(presetData.palette?.brand, null, 2));
      }
      ...
      if (demographicId === 'education') {
        console.log('DEBUG: Validation results:', JSON.stringify(results.map(r => ({ fg: r.foreground, bg: r.background, ratio: r.ratio, valid: r.valid })), null, 2));
      }
