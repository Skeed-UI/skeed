...
function StarRating({ rating }: { rating: number }) {
  return (
    <div
      className="flex items-center gap-skeed-spacing-1 mb-skeed-spacing-3"
    ...
    >
      {Array.from({ length: 5 }).map((_, i) => (
        <span
          ...
          className={cn(
            'text-base',
            i < rating ? 'text-skeed-color-warning-500' : 'text-skeed-color-neutral-300',
          )}
        >
          {i < rating ? '★' : '☆'}
        </span>
  ...
  return (
    ...
    >
      {rating !== undefined && rating > 0 && (
        <StarRating rating={Math.min(5, Math.max(1, rating))} />
      )}
      ...
      <blockquote className="mb-skeed-spacing-4">
        {variant !== 'minimal' && (
          <span
            aria-hidden="true"
            className="text-skeed-color-brand-300 font-skeed-display text-4xl leading-none select-none"
          >
            &ldquo;
          </span>
        )}
      ...
      <figcaption className="flex items-center gap-skeed-spacing-3">
        ...
        <div>
          <p className="font-skeed-body font-semibold text-skeed-color-neutral-900">{authorName}</p>
