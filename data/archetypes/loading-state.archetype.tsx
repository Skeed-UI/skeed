...
function SkeletonBlocks({ size = 'md' }: { size?: LoadingStateProps['size'] }) {
  ...
  return (
    <div className="w-full max-w-sm flex flex-col gap-skeed-spacing-2" aria-hidden="true">
      <div
        className={cn(
          'rounded-skeed-radius-1 bg-skeed-color-neutral-200 animate-pulse w-full',
          blockHeightClass,
        )}
      />
      <div
        className={cn(
          'rounded-skeed-radius-1 bg-skeed-color-neutral-200 animate-pulse',
          blockHeightClass,
          'w-4/5',
        )}
      />
...
export const LoadingState = forwardRef<HTMLElement, LoadingStateProps>(function LoadingState(
  ...
  return (
    ...
    >
      ...
      {message && <p className="font-skeed-body text-sm text-skeed-color-neutral-600">{message}</p>}
