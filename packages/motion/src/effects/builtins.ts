...
import type { EffectContext, EffectOutput } from './types.js';

/**
 * Ripple effect - expanding wave from interaction point
 */
registerEffect('ripple', (ctx, params): EffectOutput => {
 ...
 */
registerEffect('elastic', (ctx, params): EffectOutput => {
  ...
  return {
    transforms: [
      { type: 'scale', x: baseScale * (0.9 + elasticityFactor * 0.2), y: baseScale * (0.9 + elasticityFactor * 0.2) },
    ],
    springConfig: {
      mass: ctx.material.mass,
      stiffness: 150,
 ...
 */
registerEffect('glow', (ctx, params): EffectOutput => {
 ...
 */
registerEffect('magnetic', (ctx, params): EffectOutput => {
  ...
  return {
    transforms: [
      { type: 'translate', x: pullX, y: pullY },
    ],
    springConfig: {
 ...
 */
registerEffect('breathe', (ctx, params): EffectOutput => {
  ...
  return {
    transforms: [
      { type: 'scale', x: breatheScale, y: breatheScale },
    ],
    cssProperties: {
 ...
 */
registerEffect('jiggle', (ctx, params): EffectOutput => {
 ...
 */
registerEffect('curtain', (ctx, params): EffectOutput => {
  ...
  return {
    transforms: [
      { type: 'skew', x: direction === 'vertical' ? skewAngle : 0, y: direction === 'horizontal' ? skewAngle : 0 },
    ],
    cssProperties: {
 ...
 */
registerEffect('morph', (ctx, params): EffectOutput => {
 ...
 */
registerEffect('wind', (ctx, params): EffectOutput => {
  const strength = (params.strength as number) ?? 0.4;
 ...
 */
registerEffect('lift', (ctx, params): EffectOutput => {
 ...
 */
registerEffect('pulse', (ctx, params): EffectOutput => {
  ...
  return {
    transforms: [
      { type: 'scale', x: pulseScale, y: pulseScale },
    ],
    cssProperties: {
      '--motion-pulse-opacity': String(0.7 + Math.sin((ctx.timestamp / speed) * Math.PI * 2) * 0.3),
    },
    duration: speed / 2,
    easing: 'ease-in-out',
  };
...
// Export to ensure registrations happen
export {};
