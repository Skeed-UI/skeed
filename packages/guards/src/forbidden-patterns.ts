...
export const FORBIDDEN_PATTERNS: ForbiddenPattern[] = [
  // Universal dark patterns
  {
    ...
    reason: 'autoplay AV is hostile to most users',
    severity: 'warn',
  },
  {
    demographic: '*',
    pattern: /\bonly\s+\d+\s+left\b/i,
    reason: 'fake-scarcity urgency text',
    severity: 'block',
  },
  {
    demographic: '*',
    pattern: /\bact\s+now\b.*\bexpires?\b/i,
    reason: 'urgency-timer copy',
    severity: 'block',
  },
  ...
  // Kids
  {
    demographic: 'kids',
    pattern: /\bautoplay\b/i,
    reason: 'COPPA: kids must not face autoplay AV',
    severity: 'block',
  },
  {
    demographic: 'kids',
    pattern: /\binfinite\s+scroll\b/i,
    reason: 'kids must not face infinite scroll',
    severity: 'block',
  },
  {
    demographic: 'kids',
    pattern: /\bdata:image\/[^;]+;base64,[A-Za-z0-9+/]{1000,}/,
    reason: 'large embedded image — kids bundle should stay light',
    severity: 'warn',
  },
  ...
  {
    demographic: 'health',
    pattern: /\?(\w+=)?[\w-]*(ssn|patient|medical-record-number)/i,
    reason: 'PII fragment in URL',
    severity: 'block',
  },
  ...
  {
    demographic: 'gov',
    pattern: /\bclick\s+here\b/i,
    reason: 'gov plain-language: avoid "click here"',
    severity: 'warn',
  },
  ...
  // Mental wellness
  {
    ...
    pattern: /\b(streak|don\'?t\s+break|don\'?t\s+lose)\b/i,
    reason: 'streak-shame patterns are harmful in wellness context',
    severity: 'block',
  },
  {
    demographic: 'mental_wellness',
    pattern: /\b\d+\s+notifications?\b/i,
    reason: 'notification spam in wellness app',
    severity: 'warn',
  },
];
